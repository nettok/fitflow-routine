...
async fn event_processing_loop(redis_pool: RedisPool) {
    ...
    loop {
        if let Some(event) = conn.brpop("goal-set", 0f64).await.unwrap() {
            ...
            tokio::spawn(async move {
                handle_goal_set_event(event_handler_redis_pool, event).await;
            });
        }
    }
}
...
async fn handle_goal_set_event(redis_pool: RedisPool, event: [String; 2]) {
    let str_value = event[1].to_owned();
    tracing::info!("Got GoalSetEvent event: {}", str_value);
    let goal_set_event = serde_json::from_str::<GoalSetEvent>(&str_value).unwrap();
    ...
    let mut conn = redis_pool.get().await.unwrap();
    let list_length = conn.lpush("routine-recommended", event_str).await.unwrap();
    tracing::info!(
